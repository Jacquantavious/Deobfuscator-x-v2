...
async function bootstrap() {
  ...
  const objectAliasPass = {
    ...
    run(ast, { log }) {
      let inlined = 0, callsInlined = 0;
      // Detect: const obj = { a: fn1, b: 'str', c: function(x,y){ return x+y; }, ... }
      // javascript-obfuscator's "transformObjectKeys" / control-flow-flattening pass
      // commonly emits objects that mix plain literal values with tiny one-line
      // "operator wrapper" functions (e.g. {'llPDJ': (a,b) => a < b}) under random
      // property names. The previous version of this pass rejected the ENTIRE
      // object if even a single property held a function — which is precisely the
      // shape these objects have, so it silently did nothing on real-world input.
      // Now we keep whatever properties we *can* understand (per-property, not
      // per-object) and support two kinds:
      //   - 'value': plain identifier/literal -> inline directly at read sites
      //   - 'fn':    trivial single-expression wrapper function -> inline by
      //              substituting the call arguments into its body at call sites
      ...
      traverse(ast, {
        VariableDeclarator(path2) {
          ...
          for (const prop of init.properties) {
            if (!t.isObjectProperty(prop) || prop.shorthand) continue;
            const key = !prop.computed && t.isIdentifier(prop.key) ? prop.key.name
                      : t.isStringLiteral(prop.key) ? prop.key.value : null;
            if (key === null) continue;
            const val = prop.value;
            if (t.isIdentifier(val) || t.isStringLiteral(val) || t.isNumericLiteral(val) || t.isBooleanLiteral(val)) {
              props.set(key, { kind: 'value', node: val });
            } else if ((t.isFunctionExpression(val) || t.isArrowFunctionExpression(val)) &&
                       !val.async && !val.generator && val.params.every(p => t.isIdentifier(p))) {
              let bodyExpr = null;
              if (t.isBlockStatement(val.body)) {
                if (val.body.body.length === 1 && t.isReturnStatement(val.body.body[0]) && val.body.body[0].argument) {
                  bodyExpr = val.body.body[0].argument;
                }
              } else {
                bodyExpr = val.body; // arrow with expression body
              }
              if (bodyExpr) props.set(key, { kind: 'fn', params: val.params.map(p => p.name), expr: bodyExpr });
            }
            // Anything else (getters, spreads, multi-statement functions, etc.)
            // is simply left alone — it no longer poisons the rest of the object.
          }
          if (props.size > 0) aliasObjects.set(id.name, props);
        },
      });
      ...
      function substitute(node, paramMap) {
        ...
        if (Array.isArray(node)) { for (let i = 0; i < node.length; i++) node[i] = substitute(node[i], paramMap); return node; }
        if (node.type === 'Identifier' && paramMap.has(node.name)) return t.cloneNode(paramMap.get(node.name));
        for (const key of t.VISITOR_KEYS[node.type] || []) {
          if (node[key] != null) node[key] = substitute(node[key], paramMap);
        }
        return node;
      }

      // Call sites first: obj.key(args) -> substituted wrapper-function body
      traverse(ast, {
        CallExpression(path2) {
          ...
          if (!alias) return;
          const key = !callee.computed && t.isIdentifier(prop) ? prop.name : t.isStringLiteral(prop) ? prop.value : null;
          if (key === null) return;
          const entry = alias.get(key);
          if (!entry || entry.kind !== 'fn') return;
          const args = path2.node.arguments;
          if (args.length !== entry.params.length || args.some(a => t.isSpreadElement(a))) return;
          const paramMap = new Map(entry.params.map((p, i) => [p, args[i]]));
          const clone = substitute(t.cloneNode(entry.expr, true), paramMap);
          path2.replaceWith(clone);
          callsInlined++;
        },
      });

      // Plain reads: obj.key -> literal value
      traverse(ast, {
        MemberExpression(path2) {
          if (t.isCallExpression(path2.parent) && path2.parent.callee === path2.node) return; // handled above
          const obj = path2.node.object, prop = path2.node.property;
          if (!t.isIdentifier(obj)) return;
          const alias = aliasObjects.get(obj.name);
          if (!alias) return;
          const key = !path2.node.computed && t.isIdentifier(prop) ? prop.name : t.isStringLiteral(prop) ? prop.value : null;
          if (key === null) return;
          const entry = alias.get(key);
          if (!entry || entry.kind !== 'value') return;
          if (t.isAssignmentExpression(path2.parent) && path2.parent.left === path2.node) return;
          path2.replaceWith(t.cloneNode(entry.node));
          inlined++;
        },
      });

      if (inlined + callsInlined > 0) log('Inlined ' + inlined + ' alias value(s) and ' + callsInlined + ' wrapper-function call(s)');
  ...
  const extendedUnicodeNormPass = {
    ...
    run(ast, { log }) {
      ...
      // Single-pass iterative AST walk immune to Babel's circular back-references.
      // We combine collection + application into one walk:
      //   - On first encounter of an Identifier with a non-ASCII name, build the
      //     rename mapping and immediately apply it to this node.
      //   - On subsequent encounters of the same name, the mapping is already in
      //     renameMap so we skip extNormalize and just apply.
      // This halves the number of full-tree traversals vs. the two-pass approach.
      //
      // Key performance choices:
      //   1. visited WeakSet — handles Babel's circular path/scope/hub back-refs.
      //   2. Object.keys() — own enumerable keys only; never walks prototype chain.
      //   3. Skip-list of known non-AST fields checked with a Set (O(1) per key).
      //   4. hasNonAscii() fast-path skips extNormalize for already-clean names.
      //   5. Prototype guard rejects non-plain-object children (Babel internals).

      const SKIP_KEYS = new Set([
        'type','start','end','loc','range','extra','tokens','errors',
        'comments','leadingComments','trailingComments','innerComments',
        'name','value','operator',
      ]);
      ...
      while (stack.length > 0) {
        ...
        // Process Identifier nodes inline
        if (node.type === 'Identifier') {
          const name = node.name;
          if (hasNonAscii(name)) {
            let candidate = renameMap.get(name);
            if (candidate === undefined) {
              // Not yet seen — compute and register the mapping
              ...
              if (normalized && normalized !== name && ASCII_IDENT_RE.test(normalized)) {
                ...
                // Mark as non-renameable so we don't re-evaluate next encounter
                renameMap.set(name, null);
              }
        ...
        // Push plain-object/array children onto the stack
        const keys = Object.keys(node);
        for (let i = 0; i < keys.length; i++) {
          const key = keys[i];
  ...
  reg.registerAll([
    runtimePatternPass, zeroxDecoderPass, sandboxDecoderPass, stringArrayCleanupPass,
