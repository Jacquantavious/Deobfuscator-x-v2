...
// Resolve worker.js path relative to this script, works in any subpath on GitHub Pages
const WORKER_URL = (() => {
  const scripts = document.querySelectorAll('script[src]');
  for (const s of scripts) {
    if (s.src.includes('script.js')) return s.src.replace('script.js', 'worker.js');
  }
  return new URL('worker.js', import.meta?.url ?? location.href).href;
})();
