...
//
// waitForMonacoRequire() polls until the global `require` has been replaced
// by Monaco's AMD loader.  Monaco's loader.js replaces the plain config-object
// stub with a callable function and exposes `require.config` (and optionally
// `require.specified`).  We accept any of several signals so the check works
// across Monaco versions and CDN timing variations.
// ════════════════════════════════════════════════════════════════════════════

function waitForMonacoRequire(timeout = 15000) {
  return new Promise((resolve, reject) => {
    const deadline = Date.now() + timeout;

    function isLoaderReady() {
      // The global `require` must be a callable function (not the plain config
      // object that index.html seeds before loader.js runs).
      if (typeof require !== 'function') return false;
      // Monaco's AMD loader exposes at least one of these methods.
      // require.config  — always present in Monaco's loader
      // require.specified — present in some versions
      // require.defined  — present in some versions
      return (
        typeof require.config    === 'function' ||
        typeof require.specified === 'function' ||
        typeof require.defined   === 'function'
      );
    }

    function check() {
      if (isLoaderReady()) { resolve(); return; }
      if (Date.now() >= deadline) {
        reject(new Error('Monaco loader.js did not install its AMD require within ' + timeout + 'ms'));
        return;
      }
      setTimeout(check, 20);
    }
...
class EditorManager {
  ...
  async init({ inputEl, outputEl }) {
    // Bug 3 fix: wait for loader.js to finish installing AMD require before
    // calling require([...]).  Without this, require may still be the plain
    // config object and the call throws / silently does nothing.
    await waitForMonacoRequire();
